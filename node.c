...
int main(int argc, char **argv) {
  ...
  if (receiveMessage(sockfd, (void *)&response) == -1) {
    // Timeout happened -> call election
    message msg;
    vectorClock clock[MAX_NODES];

    msg.electionID = rand();
    msg.msgID = ELECT;
    memcpy(msg.vectorClock, nodeTimes, sizeof(nodeTimes));

    sendMessage(&msg, hostname, sockfd, argv[1], serverAddr);
  } else {
    // Received a message -> send message back.
    sendMessage(&response, hostname, sockfd, argv[1], serverAddr);
  }
