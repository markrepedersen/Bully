...
/* This is the message sent between nodes. Simply fill this structure
   ...
   are to be sent in the network byte order for their respective
   types. The electionID value is to be selected by a node when it
   starts an election. Each election a node starts is to have a
   different election ID. Although it would be helpful if the election
   IDs were different across all nodes that start elections, it is not
   a requirement.  If a node is forwarding the election call then this
   value is left unchanged. The election ID has no role in the actual
   election and is to be used for logging purposes only. If a time
   value for a node is not known, it is set to 0. Initially a node
   only knows its own clock time, but it will learn the clock values
   at other nodes as messages are exchanged. If there are fewer nodes
   in the system than vectorClock slots then the node ID for the
   ...
   is possible and that unused slots could be encountered anywhere
